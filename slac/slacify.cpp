...
extern "C" {
#include "../slac/plc/plc.h"
#include "../slac/slac/slac.h"
#include "../slac/tools/error.h"
#include "../slac/tools/flags.h"
#include "../slac/tools/memory.h"
#include "../slac/tools/number.h"
#include "../slac/tools/slac_config.h"
...

{
	...
	{
		// sample_J1772();
		// if ((J1772.pilot_state == A1) || (J1772.pilot_state == A2))
		// {
		// 	session->state = EVSE_STATE_UNAVAILABLE;
		// 	debug (0, __func__, "Coupler Removed!");
		// 	return;
		// }

		// if(J1772.prox_state !=  PLUGGED_S3_CLOSED)
		// {
		// 	debug (0, __func__, "EV Asleep or issues with Proximity");
		// 	printf("Prox State: %d\n", J1772.prox_state);
		// 	//printf("Prox V: %.2f\n", J1772.Vprox);
		// }
		...
		if(DeltaTime >= slac_timeout_sec) /*allow EVSE 20 Seconds to receive CM_SLAC_PARAM.REQ*/
		{
			...
			// bitbucket = system(PSTATE_E); /*place pilot in State E, physically removes pilot gen circuit from pilot pin*/
			sleep(5);  /*wait 10 sec*/
			// bitbucket = system(SWITCHBACKP); //switch back to pilot gen circuit
			if(C_sequ_retry >= num_retries)
			{
			// 	bitbucket = system(PWMOFF); /*place pilot in State B1*/
				session->state = EVSE_STATE_UNAVAILABLE;
				C_sequ_retry = 0; /*clear flag*/
				// SLACTIMEOUT = 1;  /*set this flag so we don't call the SLAC function until unplug/replug*/
			// 	hlc_state = SHUTDOWN_Connected;
			// 	/*Show Message on EVSE Screen*/
			}
			else
			{
			//   bitbucket = system(PWMON_5); /*place pilot in State B2*/
			  session->state = EVSE_STATE_UNOCCUPIED;
			  C_sequ_retry++;
			}
			return;
...

{
	...
	while (pev_cm_slac_param (session, channel, message))
	{
			...
			// SampleJ1772();
			// if (j1772.PROXSTATE == UNPLUGGED)
			// {
			// 	session->state = PEV_STATE_UNAVAILABLE;
			// 	debug (0, __func__, "Connector Removed!");
			// 	return;
			// }
...

{
	...
	/*code to change NMK and NID back to original*/
/*#if SLAC_FORMAVLN
...
int PEVSlac(signed c)
{
	...
	// struct session session;
	// struct message message;
	// char const * profile = PEV_PROFILE;
	...
	/*Uncomment to set key specified in #defines above*/
	/*if (pev_cm_set_key (&session, &channel, &message))
	{
...
int CSLACify::init(int device_type, signed slac_setting, int slac_limit, int timeout, int retries)
{
    ...
    else if (m_device_type == PEV)
    {
		...
		// if (pev_cm_set_key (&Session, &channel, &Message))
		// {
		// 	debug (1, __func__, "Can't set PEV key");
		// }
		// sleep (Session.settletime);
        return 1;
    }
