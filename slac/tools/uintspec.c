...

{
	...
	if (*number) 
	{
		error (1, EINVAL, "Have '%s' but need base %d integer", string, radix);
	}
	if ((value < minimum) || (value > maximum)) 
	{
		...
		error (1, EINVAL, "Have '%s' but need %I64d thru %I64d", string, minimum, maximum);

#else

		error (1, EINVAL, "Have '%s' but need %lld thru %lld", string, minimum, maximum);
